...
void output_all_instructions(string& src,
                             string& dst,
                             int src_i,
                             int dst_i,
                             bool should_push,
  ...
  if (prefix.size() == 2 * src.size()) {
    ...
    if (input_count < int(src.size())) {
      output_all_instructions(src, dst, src_i, dst_i, true, buffer, input_count,
                              output_count, prefix);
    }
    if (output_count < int(src.size())) {
      output_all_instructions(src, dst, src_i, dst_i, false, buffer,
                              input_count, output_count, prefix);
    }
  }
}
...
int main() {
  while (true) {
    ...
    cout << "[\n";
    output_all_instructions(src, dst, 0, 0, true, buffer, 0, 0, instructions);
    cout << "]\n";
  }
}
