...
void handle_weighing(unordered_map<char, Coin>& coins, sc& left, sc& right, char& weighing_result) {
    sc met_coin_ids;
    for (auto& coin_id : left) met_coin_ids.insert(coin_id);
    for (auto& coin_id : right) met_coin_ids.insert(coin_id);
