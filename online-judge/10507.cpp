...
int main() {
    while (true) {
        ...
        for (int j = 0; j < m; ++j) {
            ...
            if (connections.find(a) == connections.end()) connections[a] = {};
            if (connections.find(b) == connections.end()) connections[b] = {};
        ...
        // worst case scenario: each year an area wakes up (out of (n-3) sleeping areas)
        while (true) {
