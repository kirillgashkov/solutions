...
bool can_such_child_exist(map<char, int>& mother_counts, map<char, int>& father_counts, map<char, int>& child_counts) {
    ...
    for (auto& possible_child_counts : all_possible_child_counts) {
        bool is_possible = possible_child_counts['A'] == child_counts['A']
                and possible_child_counts['B'] == child_counts['B']
                and possible_child_counts['O'] == child_counts['O']
                and possible_child_counts['+'] == child_counts['+']
                and possible_child_counts['-'] == child_counts['-'];
...
int main() {
    ...
    while (true) {
                    ...
                    if (is_child_mystery) {
                        for (auto& father_combo : father_combos) {
                            bool is_possible = can_such_child_exist(
                                    mother_combo.allele_counts,
                                    father_combo.allele_counts,
        ...
        if (is_child_mystery) {
            cout << mother_blood << ' ' << father_blood << ' ' << format_mystery_bloods(possible_mystery_bloods);
        } else if (did_swap_parents) {
            cout << format_mystery_bloods(possible_mystery_bloods) << ' ' << mother_blood << ' ' << child_blood;
        } else {
            cout << mother_blood << ' ' << format_mystery_bloods(possible_mystery_bloods) << ' ' << child_blood;
        }
        cout << '\n';
    }
}
