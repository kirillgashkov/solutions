...
bool is_palindrome(string& word) {
    ...
    for (int i = 0; i < ceil(word.size() / 2.0); ++i) {
        if (word[i] != word[last_i - i]) return false;
    }
...
bool is_mirrored(string& word) {
    ...
    for (int i = 0; i < ceil(word.size() / 2.0); ++i) {
        if (MIRROR_LETTERS.find(word[i]) == MIRROR_LETTERS.end()) return false;
        if (MIRROR_LETTERS.at(word[i]) != word[last_i - i]) return false;
    }
